...
# ═══════════════════════════════════════
#   LAPTOP 3 - Nodo normal
# ═══════════════════════════════════════
# Solo cambia estas IPs:
MI_IP        = "192.168.1.12"   # <-- pon tu IP aqui (ipconfig en windows)
IP_LAPTOP1   = "192.168.1.10"   # <-- IP de Laptop 1
IP_LAPTOP2   = "192.168.1.11"   # <-- IP de Laptop 2
PUERTO       = 9000
...
print("="*45)
print("   LAPTOP 3 - RED P2P JERARQUICA")
print("="*45)
...
def buscar_supernodo():
    global supernodo_ip
    for ip in [IP_LAPTOP1, IP_LAPTOP2]:
        try:
            s = socket.socket()
            s.settimeout(2)
            s.connect((ip, PUERTO))
...
def monitor():
    while True:
        ...
        if not buscar_supernodo():
            ...
            time.sleep(3)  # Laptop 3 espera un poco mas que Laptop 2
            if not buscar_supernodo():
                ser_supernodo()
